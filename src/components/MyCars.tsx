...
export function MyCars() {
  ...
  async function handleRentalStatus(rentalId: string, status: 'approved' | 'rejected') {
    try {
          ...
          {cars.map((car) => (
            <div key={car.id} className="bg-white rounded-lg shadow-md overflow-hidden">
              <div className="p-6">
                ...
                {/* Rental Requests */}
                <div className="mt-6">
                  <h4 className="font-semibold text-gray-900 mb-4">Rental Requests</h4>
                  {rentals[car.id]?.length > 0 ? (
                          ...
                          <div className="flex justify-between items-start">
                            ...
                            {rental.status === 'pending' && (
                              <div className="flex space-x-2">
                                <button
                                  onClick={() => handleRentalStatus(rental.id, 'approved')}
                                ...
                                <button
                                  onClick={() => handleRentalStatus(rental.id, 'rejected')}
                            ...
                            {rental.status !== 'pending' && (
                              <span className={`px-3 py-1 rounded-md text-sm ${
                                rental.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                              }`}>
                    ...
                    <p className="text-gray-600 text-sm">No rental requests yet.</p>
                  )}
                </div>
              </div>
            </div>
